...
public class DataConvertor {
	...
	public List<LocalMarker> load(String url, String rawResult, DataSource ds){
		...
		try {
			...
			*/
			Log.v(MixView.TAG, "error" + e.toString());
		}
		return null;
	}
...
//					if(url.toLowerCase().contains(urlIdentifier.toLowerCase())){
					if(url.toLowerCase(Locale.ENGLISH).contains(urlIdentifier.toLowerCase(Locale.ENGLISH))){
						return dp;
					}
				}
