...
import gr.telesto.data.smart.SmartExtraData;
...
//	private MixVector oMarker = new MixVector();
	...
	// extend Local Marker with smart Data
	public SmartExtraData extraData = new SmartExtraData();
...
//	}
	...
	public boolean fClick(float x, float y, MixContextInterface ctx, MixStateInterface state) {
		...
		if (isClickValid(x, y)) {
			Log.i("LocalMarker", "fClick");
			evtHandled = state.handleEvent(ctx, URL);
		}
		return evtHandled;
	}
	
	public boolean isClicked (float x, float y, MixContextInterface ctx, MixStateInterface state) {
		boolean evtHandled = false;
	 ...
	 */
	public SmartExtraData getExtraData() {
		return extraData;
	}
	 ...
	 */
	public void setExtraData(SmartExtraData extraData) {
